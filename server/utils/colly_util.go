...
func CollectArticle(keyWord string) {
	...
	collector.OnHTML("h3", func(e *colly.HTMLElement) {
		...
		//	title := strings.TrimSpace(element.Text) // 移除多余的空格
		//	item.Title = title
		//})
		//
		//// 提取作者
		//subCollector.OnHTML("span._2gGWi", func(element *colly.HTMLElement) {
		//	author := strings.TrimSpace(element.Text)
		...
		//
		//	publishTime := strings.TrimSpace(element.Text)
		...
		if strings.Contains(articleUrl, "baijiahao") {
			articleUrlArr := strings.Split(articleUrl, "&")

			time.Sleep(5 * time.Second)
			// 访问文章URL
			subCollector.Visit(articleUrlArr[0])
		}
